...
| googleHealth.serviceAccountKey | string | `""` | Google service-account JSON used to manage the subscriptions. Without it, subscription management stays inactive. The chart mounts it at a fixed path, so only the key itself is configurable. |
